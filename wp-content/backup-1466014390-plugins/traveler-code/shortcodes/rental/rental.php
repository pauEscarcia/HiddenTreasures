...
if(!function_exists('st_rental_add_review'))
{
    function st_rental_add_review()
    {
        if(is_singular('st_rental'))
        {
            return '<div class="text-right mb10"><a class="btn btn-primary" href="'.get_comments_link().'">'.__('Write a review',STP_TEXTDOMAIN).'</a>
                                        </div>';
        }
    }
}
...
add_shortcode('st_search_rental_result','st_search_rental_result');
add_shortcode('st_search_rental_title','st_search_rental_title');

add_shortcode('st_rental_book_form','st_rental_book_form');
add_shortcode('st_rental_header','st_rental_header');
add_shortcode('st_rental_video','st_rental_video');
add_shortcode('st_rental_price','st_rental_price');
add_shortcode('st_rental_add_review','st_rental_add_review');
add_shortcode('st_rental_nearby','st_rental_nearby');
add_shortcode('st_rental_review','st_rental_review');
add_shortcode('st_rental_review_detail','st_rental_detail_review_detail');
add_shortcode('st_rental_review_summary','st_rental_detail_review_summary');
add_shortcode('st_rental_map','st_rental_detail_map');