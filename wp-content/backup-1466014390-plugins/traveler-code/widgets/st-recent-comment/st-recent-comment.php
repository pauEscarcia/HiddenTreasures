...
class st_recent_comment extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct('recent-comments', __('ST Recent Comments',STP_TEXTDOMAIN));
        ...
        if ( $comments ) {
            ...
            foreach ( (array) $comments as $comment) {
                ...
                $output.='<div class="thumb-list-item-caption">
                        <p class="thumb-list-item-meta"><a href="'.esc_url( get_comment_link( $comment->comment_ID ) ).'">'.human_time_diff( get_comment_time('U'), current_time('timestamp') ) . __(' ago',STP_TEXTDOMAIN).'</a> </p>
                        <h4 class="thumb-list-item-title">
                        '.sprintf( __( '%s', STP_TEXTDOMAIN ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ).'
                        </h4>
                </div>';
